...
def logo_path() -> Path:
    """Return logo path near executable/script so user can drop KotR.png manually."""
    executable_dir = Path(sys.executable).resolve().parent
    script_dir = Path(__file__).resolve().parent

    for candidate in (
        executable_dir / "KotR.png",
        script_dir / "KotR.png",
        Path.cwd() / "KotR.png",
        resource_path("KotR.png"),
    ):
        if candidate.exists():
            return candidate

    return executable_dir / "KotR.png"
...
class InstallerApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(left, text="MewRus Installer", style="Title.TLabel").pack(anchor="w")
        ttk.Label(
            left,
        ...
        image_holder = ttk.Frame(top, style="Card.TFrame")
        image_holder.pack(side="right", anchor="ne")
        self._build_logo(image_holder)
    ...
    def _build_logo(self, parent: ttk.Frame) -> None:
        logo = logo_path()
        if logo.exists():
            self.kotr_image = tk.PhotoImage(file=logo)
            ttk.Label(parent, image=self.kotr_image, style="Body.TLabel").pack(anchor="ne")
        else:
            ttk.Label(parent, text="KotR.png\nне найден", style="Body.TLabel", justify="center").pack(anchor="ne")
